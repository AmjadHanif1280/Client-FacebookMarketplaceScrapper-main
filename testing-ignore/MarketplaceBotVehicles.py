...
def send_discord_msg(
    ...
    requests.post(WEBHOOK_URL, json=data, timeout=None)
...
while True:
    ...
    for query in queries:
        ...
        resp = requests.get(
            f"https://dev-fb-mk-bot.aidan4043.repl.co/search?locationLatitude={lat}&locationLongitude={long}&listingQuery={query.lstrip()}",
            timeout=None,
        ).json()
        ...
        if len(resp["data"]["listingPages"][0]["listings"]) >= 3:
            listings = resp["data"]["listingPages"][0]["listings"][:3]
        else:
            listings = resp["data"]["listingPages"][0]["listings"]
        ...
        for listing in listings:
            ...
            # save the markdown and image to a folder called "results" if there is stuff in the files just append to it:
            # FileNotFoundError: [Errno 2] No such file or directory: 'results/548182787240837.md':
